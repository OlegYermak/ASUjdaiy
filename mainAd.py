...
class MainWindow(tk.Tk):
    ...
    def setWidgets(self):
        ...
        self.editBtn = tk.Button(
            self.bottomFrame, text="Внести дані", font=('Arial', 20, 'bold'))

        self.editBtn.grid(row=0, column=1, sticky='we', padx=[50, 30])
        self.editBtn.config(command=self.callEditWindow)

        self.reportBtn = tk.Button(
            self.bottomFrame, text="Звіти/графіки", font=('Arial', 20, 'bold'))

        self.reportBtn.grid(row=0, column=2, sticky='we', padx=[50, 30])
        self.reportBtn.config(command=self.callReportWindow)
        
        self.rozrB = tk.Button(
            self.bottomFrame, text="Редагувати ОЗ", font=('Arial', 10, 'bold'))
    ...
    def rozrBtn(self):
        subprocess.Popen(["python", "dbCh.py"], creationflags=subprocess.CREATE_NO_WINDOW)
